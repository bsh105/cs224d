...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      L = tf.Variable(tf.random_uniform([len(self.wv), self.config.embed_size], -1.0, 1.0))
      window = tf.nn.embedding_lookup(L, self.input_placeholder)
      window = tf.reshpae(window, (-1, self.config.window_size * self.config.embed_size))
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope("Layer"):
        ...
        W = xavier_initializer(shape)
        b1 = tf.Variable(tf.zeros([self.config.hidden_size]))

    with tf.variable_scope("Softmax"):
        xavier_initializer = xavier_weight_init()
        shape = (self.config.hidden_size, self.config.label_size)
        U = xavier_initializer(shape)
        b2 = tf.Variable(tf.zeros([self.config.label_size]))

    self.WW = tf.multiplication(W, W)
    self.UU = tf.multiplication(U, U)
    
    h = tf.tanh(tf.matmul(window, W) + b1)
    h_drop = tf.nn.dropout(h, self.dropout_placeholder)
    yhat = tf.nn.softmax(tf.matmul(h_drop, U) + b2)
    output = yhat
